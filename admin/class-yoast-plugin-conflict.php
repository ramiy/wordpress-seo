...
class Yoast_Plugin_Conflict {
	...
	public function get_conflicting_plugins_as_string( $plugin_section ) {
		...
		foreach ( $plugins as $plugin ) {
			if ( $name = $this->get_plugin_name( $plugin ) ) {
				$plugin_names[] = '<em id="wpseo-alert-' . strtolower( str_replace( array( ' ', '-', '_', ',', '.', '/', '"', "'" ), '', $name ) ) . '">' . $name . '</em>';
			}
		}
	...
	protected function set_error( $plugin_section, $readable_plugin_section ) {
		...
		$error_message .= '<p class="alignright"><small>' . __( 'This warning is generated by WordPress SEO by Yoast.', 'wordpress-seo' ) . '</small></p><div class="clear"></div>';

		// Add the message to the notifications center
		Yoast_Notification_Center::get()->add_notification( new Yoast_Notification( $error_message, 'error notice is-dismissible yoast-wpseo-conflict-alert' ) );
	}
